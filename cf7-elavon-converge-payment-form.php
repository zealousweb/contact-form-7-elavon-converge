...
function accept_payment_using_elavon_enable_payment_gateway() {
	$form_id = intval(sanitize_text_field($_POST['form']));
	$enable = get_post_meta( $form_id, "_cf7elavon_use", true);
	echo $enable;
	wp_die();
}
add_action( 'wp_ajax_enable_elavon_gateway', 'accept_payment_using_elavon_enable_payment_gateway' );
add_action( 'wp_ajax_nopriv_enable_elavon_gateway', 'accept_payment_using_elavon_enable_payment_gateway' );


add_action( 'wp_ajax_elavon_payment_gateway', 'accept_payment_using_elavon_payment_gateway_form' );
add_action( 'wp_ajax_nopriv_elavon_payment_gateway', 'accept_payment_using_elavon_payment_gateway_form' );

function accept_payment_using_elavon_payment_gateway_form() {
	...
	$formCustomData = sanitize_text_field( $_POST['form']);

	if(isset( $formCustomData ))
	{	
		$form_id = sanitize_text_field($formdata['_wpcf7']);

		$apimode = get_post_meta( $form_id, "_cf7elavon_modes", true);
		if($apimode == '1'){
			$url_field = 'https://api.demo.convergepay.com/VirtualMerchantDemo/processxml.do';	
		} else {
			$url_field = 'https://api.convergepay.com/VirtualMerchant/processxml.do';
		}
...
add_action( 'wp_ajax_elavon_sendEmailToUser', 'accept_payment_using_elavon_get_elavon_sendEmailToUser' );
...
add_action( 'wp_ajax_elavon_seralize_data', 'accept_payment_using_elavon_seralize_data_form' );
...
add_action('wpcf7_before_send_mail', 'accept_payment_using_elavon_before_send_mail');
function accept_payment_using_elavon_before_send_mail($WPCF7_form) {
...
add_filter( 'wpcf7_validate_text', 'accept_payment_using_elavon_text_custom_validation_message', 20, 2 );
add_filter( 'wpcf7_validate_text*', 'accept_payment_using_elavon_text_custom_validation_message', 20, 2 );
 
function accept_payment_using_elavon_text_custom_validation_message( $result, $tag ) {
    ...
    if ( $amount_field == $name) {	
    	$amountField = sanitize_text_field($_POST[$amount_field]);
       if(isset( $amountField ) &&  $amountField == ''){
       		$result->invalidate( $cmtagobj, "The field is required." );	
       }else{
       		$expr = '/^[0-9.]+$/i';
       		
       		$preg_amount = isset( $amountField ) ? preg_match($expr, $amountField):'';
...
function accept_payment_using_elavon_after_send_mail($WPCF7_form) {	
	...
	if($enable == '1'){
		...
		$wpcfelavon = WPCF7_ContactForm::get_current();
	    if ($submission) {
	        $data = array();
	       	$data['posted_data'] = $submission->get_posted_data();  
	        $date = current_time("Y-m-d H:i:s");
	        //$form_id = $data['posted_data']['_wpcf7'];
	        $form_id = $wpcfelavon->id;
